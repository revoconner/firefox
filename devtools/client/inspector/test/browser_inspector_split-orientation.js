...
add_task(async function () {
  ...
  ok(
    !items.landscape.hasAttribute("aria-checked"),
    "The side-by-side item is not checked"
  );
  ok(
    !items.portrait.hasAttribute("aria-checked"),
    "The stacked item is not checked"
  );

  info("Lock the layout to side by side");
  EventUtils.synthesizeMouseAtCenter(items.landscape, {}, toolbox.win);
  await waitFor(
    () => Services.prefs.getCharPref(SPLIT_ORIENTATION_PREF) === "landscape"
  );
  ...
  is(
    items.landscape.getAttribute("aria-checked"),
    "true",
    "The side-by-side item is now checked"
  );
  EventUtils.synthesizeMouseAtCenter(items.portrait, {}, toolbox.win);
  await waitFor(
    () => Services.prefs.getCharPref(SPLIT_ORIENTATION_PREF) === "portrait"
  );
  ...
  Services.prefs.setCharPref(SPLIT_ORIENTATION_PREF, "landscape");
  await waitFor(() => inspector.splitBox.state.vert === true);
...
async function openSplitOrientationMenu(inspector, toolbox) {
  ...
  return {
    ...
    landscape: toolbox.doc.getElementById(
      "inspector-split-orientation-landscape"
    ),
    portrait: toolbox.doc.getElementById(
      "inspector-split-orientation-portrait"
    ),
  };
}
